...
class ScrapIntegrationTest {
    ...
    void 저장한_스크랩을_최신순으로_조회할_수_있다() throws Exception {
        ...
        var transcription2 = new Transcription("https://test", Map.of(LocalTime.of(0, 0), "hello"));
        ...
        ScrapCreatedResponse response1 = objectMapper.readValue(postResponse1.getResponse().getContentAsString(), ScrapCreatedResponse.class);

        final var postResponse2 = mockMvc.perform(post("/v1/scraps")
                                                 .contentType(MediaType.APPLICATION_JSON)
                                                 .content(objectMapper.writeValueAsString(new ScrapCreateRequest(savedTranscription2.getId()))))
                                         .andExpect(status().isOk())
                                         .andReturn();

        ScrapCreatedResponse response2 = objectMapper.readValue(postResponse2.getResponse().getContentAsString(), ScrapCreatedResponse.class);

        var mvcResult = mockMvc.perform(get("/v1/scraps"))
                               .andExpect(status().isOk())
                               .andReturn();
